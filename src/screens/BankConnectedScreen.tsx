...
const BankConnectionScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handlePlaidSuccess = async (result: PlaidLinkResult) => {
    try {
      setConnecting(true);
      setLoadingMessage('Processing your bank connection...');
      startLoading();
      ...
      if (!primaryAccount) {
        stopLoading();
        errorHandler.showErrorToUser('No accounts were found in your bank connection.');
        return;
      }
  ...
  const handlePlaidError = (error: any) => {
    ...
    errorHandler.handlePlaidError(error, { operation: 'plaidLink' });
